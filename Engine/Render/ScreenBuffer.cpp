...
{
    // 콘솔 버퍼 생성
    buffer = CreateConsoleScreenBuffer(GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr,
...
void ScreenBuffer::Clear() const
{
    ...
    BOOL result = FillConsoleOutputCharacterW(buffer, L' ', size.x * size.y, Vector2::Zero, &writtenCount);
...
void ScreenBuffer::Draw(const CHAR_INFO* const charInfo) const
{
    ...
    BOOL result = WriteConsoleOutputW(buffer, charInfo, size, Vector2::Zero, &rect);
