...
class CRAFT_API Renderer
{
    ...
    struct RenderCommand
    {
        ...
        // 그리기 정렬 순서. 값이 크면 우선순위 높음
        int sortingOrder = -1; // 초기화 되지 않았다는 의미로 -1
    };
  ...
  public:
    ...
    // 화면에 그릴 데이터를 전달하는 함수
    void Submit(const std::wstring& image, const Vector2& position, Color color = Color::White, int sortingOrder = 0);
