...
namespace Craft
{
    ...
    {
        charInfoArray =
            std::make_unique<CHAR_INFO[]>(bufferCount);

        sortingOrderArray =
            std::make_unique<int[]>(bufferCount);
    }
        ...
        : screenSize(screenSize)
    {
        ...
        frame =
            std::make_unique<Frame>(bufferCount);

        frame->Clear(screenSize);

        screenBufferArray[0] =
            std::make_unique<ScreenBuffer>(
                screenSize
            );
    ...
    void Renderer::Present()
    {
        ...
        currentBufferIndex =
            1 - currentBufferIndex;
    }

    const ScreenBuffer* const
        Renderer::GetCurrentBuffer() const
    {
        return screenBufferArray[
            currentBufferIndex
        ].get();
    }
}